...
function logFailedRecord(fileName: string, reason: string, data: unknown): void {
...
interface IBusinessData {
  ...
  核准設立日期?: IDateField | null; // Info: (20250801-Tzuhan) establishedDate
  最近異動日期?: IDateField | null; // Info: (20250801-Tzuhan) lastChangeDate
...
interface ICompanyData {
  ...
  核准設立日期?: IDateField | null; // Info: (20250801-Tzuhan) establishedDate
  最近異動日期?: IDateField | null; // Info: (20250801-Tzuhan) lastChangeDate
  最後核准變更日期?: IDateField | null; // Info: (20250801-Tzuhan) lastApprovedChange
  公司狀況日期?: IDateField | null; // Info: (20250801-Tzuhan) statusDate
  ...
  董監事名單?: IDirectorItem[]; // Info: (20250801-Tzuhan) directors (Json)
  經理人名單?: IManagerItem[]; // Info: (20250801-Tzuhan) managers (Json)
  股權狀況?: string; // Info: (20250801-Tzuhan) shareholdingStatus
  '停業日期(起)'?: IDateField | null; // Info: (20250801-Tzuhan) suspensionStartDate
  '停業日期(迄)'?: IDateField | null; // Info: (20250801-Tzuhan) suspensionEndDate
...
interface IBranchOfficeData {
  ...
  核准設立日期?: IDateField | null; // Info: (20250801-Tzuhan) → established_date
  最後核准變更日期?: IDateField | null; // Info: (20250801-Tzuhan) → last_approved_change
  ...
  廢止日期?: IDateField | null; // Info: (20250801-Tzuhan) → suspension_end_date（或另字段）
}
...
function parseDate(obj: IDateField | string | null | undefined): Date | null {
  if (!obj) return null;
  if (typeof obj === 'string') return new Date(obj);
  if ('year' in obj && obj.year > 0) return new Date(obj.year, obj.month - 1, obj.day);
  return null;
}
...
function normalizeRegNo(value: string | number | null | undefined): string | null {
  if (value === null || value === undefined) return null;
...
function parseRepresentative(value: string | string[] | undefined | null): string | null {
...
async function importFile(filePath: string): Promise<{ total: number; success: number }> {
    ...
    try {
      ...
      if ('商業名稱' in jsonData) {
        record = {
          ...
          ...(jsonData['出資額(元)'] ? { contributions: jsonData['出資額(元)'] } : {}),
        ...
        record = {
          ...
          ...(jsonData.所營事業資料 ? { businessItems: jsonData.所營事業資料 } : {}),
          status: jsonData.公司狀況 || jsonData.登記現況 || null,
          foreignCompanyName: jsonData.章程所訂外文公司名稱 || null,
          ...(jsonData.董監事名單 ? { directors: JSON.stringify(jsonData.董監事名單) } : {}),
          ...(jsonData.經理人名單 ? { managers: JSON.stringify(jsonData.經理人名單) } : {}),
      ...
      } else if ('名稱' in jsonData && '類別' in jsonData && '來源' in jsonData) {
        record = {
          name: jsonData.名稱,
          registrationNo: regNo,
          registrationCountry: 'Taiwan',

...
if [[ "$OS_TYPE" == "Darwin" ]]; then
    # Info: (20251015 - Tzuhan) macOS (BSD) date
    echo "🍎 偵測到作業系統為 macOS"
...
# Info: (20251015 - Tzuhan) 2a: 優先從資料庫查詢最新日期
...
# Info: (20251015 - Tzuhan) 2b: 若資料庫為空，則掃描本地資料夾找尋最新檔案
...
# Info: (20251015 - Tzuhan) 2c: 若資料庫和本地資料夾都為空，則使用預設起始日
if [ -z "$start_date" ]; then
    start_date=$default_start
    echo "🆕 找不到任何現有資料。將從預設日期 $start_date 開始下載。"
fi

# Info: (20251015 - Tzuhan) 主迴圈

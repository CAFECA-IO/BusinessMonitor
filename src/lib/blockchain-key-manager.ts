...
export async function createAndEncryptBlockchainKey(userId: string) {
  console.log('[DEBUG] --- createAndEncryptBlockchainKey ---');
  try {
    // Info: (20251020 - Tzuhan) 1. 生成一個全新的、真正隨機的以太坊錢包
    const wallet = ethers.Wallet.createRandom();
    console.log('[DEBUG] 1. 原始 Private Key:', wallet.privateKey);

    // Info: (20251020 - Tzuhan) 2. 構造包含 "cafeca" 前綴和意圖的 Challenge
    const nonce = ethers.hexlify(ethers.randomBytes(32));
    const derivationChallenge = `cafeca-key_encryption-${userId}-${nonce}`;
    console.log('[DEBUG] 2. 加密 Challenge:', derivationChallenge);

    // --- (A) 驗證簽章是否穩定 ---
    console.log('[DEBUG] 3a. 正在請求第一次簽章 (用於確定性檢查)');
    ...
    console.log('[DEBUG] 3b. 第一次簽章 Signature:', signature1);

    console.log('[DEBUG] 3c. 正在請求第二次簽章 (用於確定性檢查)');
    const authentication2 = await fido2ClientService.startLogin({
      challenge: derivationChallenge,
      userVerification: 'required',
    });
    const signature2 = authentication2.response.signature;
    console.log('[DEBUG] 3d. 第二次簽章 Signature:', signature2);

    if (signature1 === signature2) {
      console.log('[DEBUG] ✅ 簽章是確定的！');
    } else {
    ...
    console.log('[DEBUG] 4a. 加密 Key Material (Keccak Hash):', encryptionKeyMaterial);
    const encryptionKey = new Uint8Array(ethers.getBytes(encryptionKeyMaterial));
    console.log('[DEBUG] 4b. 加密 AES Key (Uint8Array):', encryptionKey);
    const iv = window.crypto.getRandomValues(new Uint8Array(12));
    console.log('[DEBUG] 4c. 加密 IV (Hex):', ab2hex(iv.buffer));
    ...
    console.log('[DEBUG] 5. 加密 Encrypted Data (Hex):', ab2hex(encrypted));
    ...
    console.log('[DEBUG] --- createAndEncryptBlockchainKey END ---');
  }
}
...
export async function decryptAndUseBlockchainKey(
  ...
  try {
    ...
    console.log('[DEBUG] 2. 解密 Signature:', signature);

    // Info: (20251020 - Tzuhan) 3. 派生出完全相同的對稱加密金鑰
    const encryptionKeyMaterial = ethers.keccak256(ethers.toUtf8Bytes(signature));
    console.log('[DEBUG] 3a. 解密 Key Material (Keccak Hash):', encryptionKeyMaterial);
    const encryptionKey = new Uint8Array(ethers.getBytes(encryptionKeyMaterial));
    console.log('[DEBUG] 3b. 解密 AES Key (Uint8Array):', encryptionKey);

    // Info: (20251020 - Tzuhan) 4. 使用 Web Crypto API (AES-GCM) 解密私鑰
    console.log('[DEBUG] 4a. 解密 IV (Hex):', iv);
    console.log('[DEBUG] 4b. 解密 Encrypted Data (Hex):', encryptedData);
    ...
    console.log('[DEBUG] 4c. 解密後的 ArrayBuffer:', decrypted);
    ...
    console.log('[DEBUG] 5. 解密後 Private Key:', privateKey);

    const wallet = new ethers.Wallet(privateKey);
    console.log('[DEBUG] 創建 Wallet 成功，地址:', wallet.address);
    return wallet;
  } catch (error) {
    ...
    console.log('[DEBUG] --- decryptAndUseBlockchainKey END ---');
  }
}

  ...
  const displayTabContent = TAB_BAR_ITEMS.map((item) => {
    ...
    return (
      <button
        ...
        className={`rounded-full px-48px py-16px text-base font-medium hover:bg-tab-active hover:text-text-note ${isActive ? 'bg-button-primary text-text-invert' : 'bg-transparent text-text-secondary'}`}
      >
        {t(`DETAIL_TAB_${item}`)}
      </button>
    );
